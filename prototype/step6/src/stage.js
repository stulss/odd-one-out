...
// STEP 6 — 난이도 곡선 변경 (STEP 2부터 미뤄온 숙제)
//
// 그동안 smoothstep(t*t*(3-2t))을 썼는데, 실측해보니 앞구간이 거의 평평했다.
//   n=1 → 20.0도 / n=5 → 18.8도 / n=10 → 15.0도   (10판 동안 25%만 좁혀짐)
//   n=15 → 10.4도 / n=20 → 5.9도 / n=25 → 3.1도   (뒤에서 몰아침)
// 이 게임은 대부분 8~15단계에서 끝난다. 즉 **플레이어가 실제로 보는 구간이 평평했다.**
//
// 선형으로 바꾸면 매 판 조금씩 좁혀진다. 측정값은 STEPS.md에 남겼다.
// ※ 최종값 확정은 사람이 직접 10판+10판을 해봐야 한다. 여기서는 곡선 모양만 고쳤다.
const CURVE = 'linear';                 // 'linear' | 'smoothstep'
export const easeOf = t => CURVE === 'linear' ? t : t * t * (3 - 2 * t);

export let TUNE_T = 4.4;
export const setTuneT = v => { TUNE_T = Number(v) || 4.4; };
