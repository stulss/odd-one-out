// STEP 6 — 자동 일시정지 + 플레이 로거 + 난이도 결정
// 이 단계에서 답할 질문: "오래 켜둬도 안 망가지는가? 난이도를 데이터로 정할 수 있는가?"
//
// STEP 4에서 바뀐 것
...
//
// STEP 5에서 바뀐 것 (STEP 6)
//  - 포커스 이탈/탭 전환 시 자동 일시정지 — 돌아왔을 때 시간이 다 닳아 있으면 억울하다
//  - 난이도 곡선을 smoothstep → 선형으로 변경 (stage.js, 측정 근거는 STEPS.md)
//  - 난이도 T 슬라이더를 설정에 노출하고, 판마다 결과를 로거에 남긴다
//  - 로거는 난이도 값별 중앙값을 내준다. 최종값은 **사람이 직접 10판씩** 해서 정한다

import { makeStage, scoreOf, MAX_CELLS, TUNE_T, setTuneT, AXES } from './stage.js';
...
const fresh = () => ({ n:1, points:0, combo:0, prevAnswer:-1, reactions:[], stage:null,
                       startedAt: performance.now(), wrongStreak:0 });
let run = fresh();
...
function gameOver(){
  ...
  // 판별 결과를 남긴다. 이게 난이도 결정의 유일한 근거가 된다.
  Log.add({ tuneT: TUNE_T, stages, survivedMs: Math.round(performance.now() - run.startedAt),
            points: run.points, reason: run.wrongStreak >= 3 ? 'wrong_streak' : 'timeout' });
  refreshLog();
...
function applySettings(){
  ...
  if (Number.isFinite(store.settings.tuneT)) setTuneT(store.settings.tuneT);
  $('tuneOut').textContent = TUNE_T.toFixed(1);
  $('tuneSlider').value = TUNE_T;
...
function refreshLog(){
  $('logCount').textContent = Log.read().length;
  $('logSummary').textContent = Log.summaryText();
}
...
$('bReset').addEventListener('click', () => {
  store = Save.reset(); Log.clear(); applySettings(); updateTitle(); refreshLog();
  toast('기록을 초기화했습니다');
});
$('tuneSlider').addEventListener('input', e => applyTune(+e.target.value));
$('bCSV').addEventListener('click', () => Share.copy(Log.toCSV(), toast));

// ★ 포커스 이탈 / 탭 전환 → 자동 일시정지.
//    돌아왔을 때 시간이 다 닳아 있으면 플레이어는 자기 잘못이 아닌 실패를 당한다.
document.addEventListener('visibilitychange', () => {
  if (document.hidden && ui.is('PLAY')){ timer.pause(); ui.set('PAUSE'); status(); }
});
window.addEventListener('blur', () => {
  if (ui.is('PLAY')){ timer.pause(); ui.set('PAUSE'); status(); }
});
...
window.__step6 = {
  ...
  makeStage, AXES, Save, Log, applyTune,
  get tuneT(){ return TUNE_T; },
};
